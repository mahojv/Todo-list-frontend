...
export default function TaskItem({ task }) {
  return (
    <li className="flex justify-between pr-8 mb-1">
      <label htmlFor={task.id} className={`flex gap-2 items-center    ${task.completed === true ? "line-through" : ""}  `} >
        <input type="checkbox" id={task.id} className="" checked={task.completed} />
        <p>{task.title}</p>
      </label>
